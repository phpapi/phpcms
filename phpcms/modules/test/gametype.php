...
//        $this->db = pc_base::load_model('gametype_model');
    ...
    public function manage() {
        ...
        $pages = $this->db->pages;
        pc_base::load_sys_class('format', '', 0);
        include $this->admin_tpl('gametype_list');
    }
    ...
    public function add() {
        ...
        if(isset($_POST['dosubmit'])) {
            ...
            if(trim($_POST['info']['url'])=='') showmessage(L('link_is_empty'));
            if(trim($_POST['info']['typeid'])=='') showmessage(L('type_is_empty'));

            $this->page_db = pc_base::load_model('gametype_model');
            ...
            include $this->admin_tpl('gametype_add');
        }
...
//            $formValidator = $content_form->formValidator;
            include $this->admin_tpl('gametype_edit');
        }
        header("Cache-control: private");
    }
    ...
    public function del() {
        ...
        if ($this->page_db->delete_content($id)) {
            showmessage(L('operation_success'), '?m=test&c=gametype&a=manage');
        } else {
            showmessage(L('operation_failure'), HTTP_REFERER);
        }
    }
