...
?>
    <tr>
        ...
        <td align="left"><a href="javascript:edit(<?php echo $v['id']?>, '<?php echo  new_addslashes(new_html_special_chars($v['title']))?>')"><?php echo L('edit')?></a> |
            <?php if($v['siteid']!=1) { ?><a href="?m=test&c=gametype&a=del&id=<?php echo $v['id']?>" onclick="return confirm('<?php echo new_addslashes(new_html_special_chars(L('confirm', array('message'=>$v['name']))))?>')"><?php echo L('delete')?></a><?php } else { ?><font color="#cccccc"><?php echo L('delete')?></font><?php } ?></td>
    </tr>
<?php
	}
...
function edit(id, name) {
        window.top.art.dialog({title:'<?php echo L('edit')?>--'+name, id:'edit', iframe:'?m=test&c=gametype&a=edit&id='+id ,width:'500px',height:'400px'}, 	function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;
